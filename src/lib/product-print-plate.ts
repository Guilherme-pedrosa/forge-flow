...
export function sumProductPlateReferences(plates: ProductPrintPlateValues[]) {
  ...
  for (const plate of active) {
    const units = Number.isSafeInteger(plate.units_per_plate) && plate.units_per_plate > 0 ? plate.units_per_plate : null;
  ...
  return {
    ...
    incomplete: missingGrams || missingSeconds || missingCost,
    usesEstimate,
  };
}
